...
DtEntityInfo info2 = dt_ecs_manager_get_entity(manager, e2)

dt_ecs_manager_kill(manager, e2) //убиваем сущность, также она удаляется из всех DtEcsFilter и DtEcsPool
e2 = dt_ecs_manager_new_entity(manager); //имеет тот же индекс - 2, но другое поколоние,
...
```C
#define MY_COMPONENT(X, name)\ //объявляем макрос для инициализации и регистрации компонентов
    X(int, field1, name,(DtAttributeData) {\ //поле field1 типа int c атрибутом MyAttribute
      .attribute_name = "my_attr",\
      .data = &(MyAttribute) {...},\
    })\
    X(char*, field2, name) //поле field2 типа char* без атрибутов \
    X(DtEntity, field3, name, \ //поле field3 типа DtEntity c атрибутом типа int и char*
      (DtAttributeData) {\ 
...
FOREACH(DtEntity, e, &filter->entities.entities_iterator, { 
  //проходимся по всем сущностям, у которых есть компонент IncComponent нет компонента ExcComponent - e1 и e3
});
```
