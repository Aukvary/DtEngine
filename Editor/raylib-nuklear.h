...
#define NK_ASSERT(condition)                                                                       \
    do {                                                                                           \
        if (!(condition)) {                                                                        \
            TraceLog(LOG_ERROR, "NUKLEAR: Failed assert \"%s\" (%s:%i)", #condition, "nuklear.h",  \
                     __LINE__);                                                                    \
        }                                                                                          \
    } while (0)
#endif // NDEBUG
...
InitNuklear(int fontSize); // Initialize the Nuklear GUI context using raylib's font
NK_API struct nk_context*
InitNuklearEx(Font font, float fontSize); // Initialize the Nuklear GUI context, with a custom font
NK_API Font LoadFontFromNuklear(int fontSize); // Loads the default Nuklear font
NK_API void
UpdateNuklear(struct nk_context* ctx); // Update the input state and internal components for Nuklear
NK_API void UpdateNuklearEx(struct nk_context* ctx,
                            float deltaTime); // Update the input state and internal components for
                                              // Nuklear, with a custom frame time
...
ColorToNuklear(Color color); // Convert a raylib Color to a Nuklear color object
NK_API struct nk_colorf
ColorToNuklearF(Color color); // Convert a raylib Color to a Nuklear floating color
NK_API struct Color
ColorFromNuklear(struct nk_color color); // Convert a Nuklear color to a raylib Color
NK_API struct Color
ColorFromNuklearF(struct nk_colorf color); // Convert a Nuklear floating color to a raylib Color
NK_API struct Rectangle
RectangleFromNuklear(struct nk_context* ctx,
                     struct nk_rect rect); // Convert a Nuklear rectangle to a raylib Rectangle
NK_API struct nk_rect
RectangleToNuklear(struct nk_context* ctx,
                   Rectangle rect); // Convert a raylib Rectangle to a Nuklear Rectangle
NK_API struct nk_image TextureToNuklear(Texture tex); // Convert a raylib Texture to A Nuklear image
NK_API struct Texture
TextureFromNuklear(struct nk_image img); // Convert a Nuklear image to a raylib Texture
...
NK_API void SetNuklearScaling(struct nk_context* ctx,
                              float scaling); // Sets the scaling for the given Nuklear context
NK_API float
GetNuklearScaling(struct nk_context* ctx); // Retrieves the scaling of the given Nuklear context

// Internal Nuklear functions
NK_API float nk_raylib_font_get_text_width(nk_handle handle, float height, const char* text,
                                           int len);
NK_API float nk_raylib_font_get_text_width_user_font(nk_handle handle, float height,
...
NK_API float nk_raylib_font_get_text_width(nk_handle handle, float height, const char* text,
    ...
    if (len > 0) {
        ...
        return (float) MeasureText(subtext, (int) height) +
               height * RAYLIB_NUKLEAR_FONT_SPACING_RATIO;
    }
...
NK_API float nk_raylib_font_get_text_width_user_font(nk_handle handle, float height,
    ...
    if (len > 0) {
        ...
        // Raylib only counts the spacing between characters, but Nuklear expects one spacing to be
        // counter for every character in the string:
        return MeasureTextEx(*(Font*) handle.ptr, subtext, height,
                             height * RAYLIB_NUKLEAR_FONT_SPACING_RATIO)
                   .x +
               height * RAYLIB_NUKLEAR_FONT_SPACING_RATIO;
    }
...
NK_API struct nk_context* InitNuklearContext(struct nk_user_font* userFont) {
    ...
    struct NuklearUserData* userData =
        (struct NuklearUserData*) MemAlloc(sizeof(struct NuklearUserData));
    if (userData == NULL) {
 ...
 *
 * @param fontSize The size of the font to use for GUI text. Use 0 to use the default font size
 * of 10.
 *
 ...
 *
 * @param size The size of the font to load (optional). Provide 0 if you'd like to use the default
 * size from Nuklear.
 *
...
NK_API Font LoadFontFromNuklear(int size) {
    ...
    (void) size;
    TraceLog(LOG_ERROR, "NUKLEAR: RAYLIB_NUKLEAR_INCLUDE_DEFAULT_FONT must be defined to use "
                        "LoadFontFromNuklear()");
...
#define RAYLIB_NUKLEAR_DEFAULT_FONT_GLYPHS 95
    ...
    return LoadFontFromMemory(".ttf", RAYLIB_NUKLEAR_DEFAULT_FONT_NAME,
                              RAYLIB_NUKLEAR_DEFAULT_FONT_SIZE, size, NULL,
                              RAYLIB_NUKLEAR_DEFAULT_FONT_GLYPHS);
#endif
}
 ...
 */
NK_API Color ColorFromNuklearF(struct nk_colorf color) {
 ...
 */
NK_API struct nk_colorf ColorToNuklearF(Color color) { return nk_color_cf(ColorToNuklear(color)); }
    ...
    nk_foreach(cmd, ctx) {
        switch (cmd->type) {
            ...
            case NK_COMMAND_SCISSOR: {
                ...
                BeginScissorMode((int) (s->x * scale), (int) (s->y * scale), (int) (s->w * scale),
                                 (int) (s->h * scale));
            } break;
            ...
            case NK_COMMAND_LINE: {
                ...
                Vector2 startPos =
                    CLITERAL(Vector2){(float) l->begin.x * scale, (float) l->begin.y * scale};
                Vector2 endPos =
                    CLITERAL(Vector2){(float) l->end.x * scale, (float) l->end.y * scale};
            ...
            case NK_COMMAND_CURVE: {
                ...
                Vector2 begin =
                    CLITERAL(Vector2){(float) q->begin.x * scale, (float) q->begin.y * scale};
                Vector2 controlPoint1 =
                    CLITERAL(Vector2){(float) q->ctrl[0].x * scale, (float) q->ctrl[0].y * scale};
                Vector2 controlPoint2 =
                    CLITERAL(Vector2){(float) q->ctrl[1].x * scale, (float) q->ctrl[1].y * scale};
                Vector2 end = CLITERAL(Vector2){(float) q->end.x * scale, (float) q->end.y * scale};
                DrawSplineSegmentBezierCubic(begin, controlPoint1, controlPoint2, end,
                                             (float) q->line_thickness * scale, color);
            } break;

            case NK_COMMAND_RECT: {
                const struct nk_command_rect* r = (const struct nk_command_rect*) cmd;
                Color color = ColorFromNuklear(r->color);
                Rectangle rect = CLITERAL(Rectangle){(float) r->x * scale, (float) r->y * scale,
                                                     (float) r->w * scale, (float) r->h * scale};
                float roundness = (rect.width > rect.height)
                                      ? ((2 * r->rounding * scale) / rect.height)
                                      : ((2 * r->rounding * scale) / rect.width);
                roundness = NK_CLAMP(0.0f, roundness, 1.0f);
                if (roundness > 0.0f) {
                    // DrawRectangleRoundedLines doesn't work in the same way as
                    // DrawRectangleLinesEx and it draws the outline outside the region defined by
                    // the rectangle. To compensate for that, shrink the rectangle by the thickness
                    // plus 1 (due to inconsistencies from DrawRectangleRoundedLines):
                    rect.x += ((float) r->line_thickness) * scale + 1.0f;
                    rect.y += ((float) r->line_thickness) * scale + 1.0f;
                    rect.width =
                        NK_MAX(rect.width - (2 * ((float) r->line_thickness) * scale + 1.0f), 0.0f);
                    rect.height = NK_MAX(
                        rect.height - (2 * ((float) r->line_thickness) * scale + 1.0f), 0.0f);
#if RAYLIB_VERSION_MAJOR >= 5 && RAYLIB_VERSION_MINOR == 0
                    DrawRectangleRoundedLines(rect, roundness, RAYLIB_NUKLEAR_DEFAULT_ARC_SEGMENTS,
                                              (float) r->line_thickness * scale, color);
#else
                    DrawRectangleRoundedLinesEx(rect, roundness,
                                                RAYLIB_NUKLEAR_DEFAULT_ARC_SEGMENTS,
                                                (float) r->line_thickness * scale, color);
#endif
                } else {
            ...
            case NK_COMMAND_RECT_FILLED: {
                ...
                roundness = NK_CLAMP(0.0f, roundness, 1.0f);
                if (roundness > 0.0f) {
                    DrawRectangleRounded(rect, roundness, RAYLIB_NUKLEAR_DEFAULT_ARC_SEGMENTS,
                                         color);
                } else {
            ...
            case NK_COMMAND_RECT_MULTI_COLOR: {
                const struct nk_command_rect_multi_color* rectangle =
                    (const struct nk_command_rect_multi_color*) cmd;
                Rectangle position = {(float) rectangle->x * scale, (float) rectangle->y * scale,
                                      (float) rectangle->w * scale, (float) rectangle->h * scale};
            ...
            case NK_COMMAND_CIRCLE: {
                ...
                for (unsigned short i = 0; i < c->line_thickness; i++) {
                    DrawEllipseLines((int) (c->x * scale + c->w * scale / 2.0f),
                                     (int) (c->y * scale + c->h * scale / 2.0f),
                                     c->w * scale / 2.0f - (float) i / 2.0f,
                                     c->h * scale / 2.0f - (float) i / 2.0f, color);
                }
            } break;
            ...
            case NK_COMMAND_CIRCLE_FILLED: {
                ...
                Color color = ColorFromNuklear(c->color);
                DrawEllipse((int) (c->x * scale + c->w * scale / 2.0f),
                            (int) (c->y * scale + c->h * scale / 2.0f), (int) (c->w * scale / 2),
                            (int) (c->h * scale / 2), color);
            } break;
            ...
            case NK_COMMAND_ARC: {
                ...
                DrawRingLines(center, 0, a->r * scale, a->a[0] * RAD2DEG, a->a[1] * RAD2DEG,
                              RAYLIB_NUKLEAR_DEFAULT_ARC_SEGMENTS, color);
            } break;
            ...
            case NK_COMMAND_ARC_FILLED: {
                ...
                DrawRing(center, 0, a->r * scale, a->a[0] * RAD2DEG, a->a[1] * RAD2DEG,
                         RAYLIB_NUKLEAR_DEFAULT_ARC_SEGMENTS, color);
            } break;
            ...
            case NK_COMMAND_TRIANGLE: {
                ...
                // TODO: Add line thickness to DrawTriangleLines(), maybe via a
                // DrawTriangleLinesEx()?
            ...
            case NK_COMMAND_POLYGON: {
                ...
                struct Vector2* points = (struct Vector2*) MemAlloc(
                    (unsigned int) ((size_t) (p->point_count + 1) * sizeof(Vector2)));
                unsigned short i;
            ...
            case NK_COMMAND_POLYLINE: {
                ...
                for (unsigned short i = 0; i < p->point_count - 1; i++) {
                    Vector2 start = {(float) p->points[i].x * scale,
                                     (float) p->points[i].y * scale};
                    Vector2 end = {(float) p->points[i + 1].x * scale,
                                   (float) p->points[i + 1].y * scale};
            ...
            case NK_COMMAND_TEXT: {
                ...
                if (font != NULL) {
                    ...
                    DrawTextEx(*font, (const char*) text->string, position, fontSize,
                               fontSize * RAYLIB_NUKLEAR_FONT_SPACING_RATIO, color);
                } else {
                    DrawText((const char*) text->string, (int) (text->x * scale),
                             (int) (text->y * scale), (int) fontSize, color);
                }
            } break;
            ...
            case NK_COMMAND_IMAGE: {
                ...
                Rectangle source =
                    CLITERAL(Rectangle){(float) i->img.region[0], (float) i->img.region[1],
                                        (float) i->img.region[2], (float) i->img.region[3]};
                Rectangle dest = CLITERAL(Rectangle){(float) i->x * scale, (float) i->y * scale,
                                                     (float) i->w * scale, (float) i->h * scale};
            ...
            case NK_COMMAND_CUSTOM: {
                TraceLog(LOG_WARNING,
                         "NUKLEAR: Unverified custom callback implementation NK_COMMAND_CUSTOM");
                const struct nk_command_custom* custom = (const struct nk_command_custom*) cmd;
                custom->callback(NULL, (short) (custom->x * scale), (short) (custom->y * scale),
                                 (unsigned short) (custom->w * scale),
                                 (unsigned short) (custom->h * scale), custom->callback_data);
            } break;
...
#define NK_RAYLIB_INPUT_KEYBOARD_CHECK_NUM 16
    struct nk_raylib_input_keyboard_check checks[NK_RAYLIB_INPUT_KEYBOARD_CHECK_NUM] = {
        ...
        (struct nk_raylib_input_keyboard_check) {KEY_B, NK_KEY_TEXT_LINE_START,
                                                 (control || command)},
        ...
        (struct nk_raylib_input_keyboard_check) {KEY_A, NK_KEY_TEXT_SELECT_ALL,
                                                 (control || command)},
        (struct nk_raylib_input_keyboard_check) {KEY_LEFT, NK_KEY_TEXT_WORD_LEFT,
                                                 (control || command)},
        (struct nk_raylib_input_keyboard_check) {KEY_RIGHT, NK_KEY_TEXT_WORD_RIGHT,
                                                 (control || command)},
 ...
 */
NK_API void UpdateNuklear(struct nk_context* ctx) { UpdateNuklearEx(ctx, GetFrameTime()); }
 ...
 */
NK_API void UpdateNuklearEx(struct nk_context* ctx, float deltaTime) {
 ...
 */
NK_API struct Rectangle RectangleFromNuklear(struct nk_context* ctx, struct nk_rect rect) {
 ...
 */
NK_API struct nk_rect RectangleToNuklear(struct nk_context* ctx, Rectangle rect) {
    float scaling = GetNuklearScaling(ctx);
 ...
 */
NK_API void CleanupNuklearImage(struct nk_image img) { MemFree(img.handle.ptr); }
 ...
 *
 * @return The scale value that had been set for the Nuklear context. 1.0f is the default scale
 * value.
 */
